/**
 * SQUAD Task Database Layer
 *
 * Uses .squad/ directory with SQLite (via better-sqlite3) as source of truth.
 * Scans ~/code/PROJECT/.squad/ for project databases.
 */
 ...
 * @property {number} [relevance] - Search ranking (FTS), only set by searchTasks
 */
...
const SCHEMA_PATH = new URL('./tasks-schema.sql', import.meta.url);
/** @type {string | null} */
let _schemaSQL = null;
...
/**
 * Read SQUAD config (~/.config/squad/projects.json)
 * @returns {any|null}
 */
function readSquadConfig() {
...
/**
 * Touch .squad/last-touched to notify file watchers of a mutation.
 * @param {string} projectPath
 */
function touchLastTouched(projectPath) {
  const sentinel = join(projectPath, '.squad', 'last-touched');
...
function ensureFts(db) {
  ...
  const cols = /** @type {{ name: string }[]} */ (db.pragma('table_info(tasks)'));
  const hasLabelsText = cols.some((c) => c.name === 'labels_text');
  if (!hasLabelsText) {
 ...
 * @param {string} prefix - Project prefix (e.g. "squad")
 * @returns {string}
 */
...
/**
 * Get all projects that have SQUAD task databases (.squad/tasks.db).
 * @returns {Project[]}
 */
export function getProjects() {
  ...
  // 1. Check custom paths from SQUAD config
  const squadConfig = readSquadConfig();
  if (squadConfig?.projects) {
    for (const [name, config] of Object.entries(squadConfig.projects)) {
      ...
      if (existsSync(squadDb)) {
        projects.push({ name, path: projectPath, dbPath: squadDb });
  ...
  if (existsSync(codeDir)) {
    try {
      for (const entry of readdirSync(codeDir, { withFileTypes: true })) {
        ...
        if (existsSync(squadDb)) {
          projects.push({ name: entry.name, path: projectPath, dbPath: squadDb });
        }
      }
    } catch (error) {
 ...
 * @param {string} [options.closedAfter] - ISO date string, filter closed_at >=
 * @param {string} [options.closedBefore] - ISO date string, filter closed_at <
 * @returns {Task[]}
 */
...
function findProjectForTask(taskId, projectPath) {
  ...
  if (projectPath) {
    const squadDb = join(projectPath, '.squad', 'tasks.db');
    const dbPath = existsSync(squadDb) ? squadDb : null;
    if (dbPath) {
...
/**
 * Find a writable .squad/ DB for a project path, or by task ID prefix.
 * @param {string} projectPath
 * @returns {{ db: import('better-sqlite3').Database, project: Project, table: string }}
 */
function getWritableDb(projectPath) {
  const dbPath = join(projectPath, '.squad', 'tasks.db');
  if (!existsSync(dbPath)) {
    throw new Error(`No .squad/ database at ${projectPath}. Run initProject() first.`);
  }
  const db = openDb(dbPath);
 ...
 * @param {string|null} [opts.assignee]
 * @param {string} [opts.notes]
 * @param {string} [opts.id] - Optional explicit ID (for child tasks like squad-abc.1)
 * @param {string} [opts.command] - Command to run (default: '/squad:start')
...
export function updateTask(taskId, updates) {
  ...
  const found = findProjectForTask(taskId, /** @type {string} */ (u.projectPath));
  ...
  try {
    ...
    if (u.status === 'closed') {
      sets.push('closed_at = ?');
      params.push(now());
    } else if (u.status && u.status !== 'closed') {
    ...
    if (u.labels !== undefined) {
      ...
      const insertLabel = db.prepare('INSERT INTO labels (issue_id, label) VALUES (?, ?)');
      for (const label of /** @type {string[]} */ (u.labels)) {
...
export function getDependencyTree(taskId, options = {}) {
  ...
  try {
    ...
    db.close();
    return /** @type {Array<{id: string, title: string, status: string, priority: number, depth: number}>} */ (result);
  } catch (error) {
    db.close();
    throw error;
  }
...
export function searchTasks(query, options = {}) {
  ...
  for (const project of projects) {
    try {
      ...
      /** @type {Array<Record<string, unknown> & { relevance?: number }>} */
      let rows;
      try {
        // FTS5 MATCH query with BM25 ranking
        rows = /** @type {Array<Record<string, unknown> & { relevance?: number }>} */ (db.prepare(`
 ...
 * @param {string} query
 * @param {Object} options
 * @param {string} [options.updatedAfter]
 * @param {string} [options.status]
...
function _searchLike(db, table, query, options) {
  const { updatedAfter, status, type, labels, limit = 50 } = /** @type {{ updatedAfter?: string, status?: string, type?: string, labels?: string[], limit?: number }} */ (options);
...
/**
 * Initialize a project's .squad/ directory with empty database.
 * @param {string} projectPath - Absolute path to the project
 * @returns {{ dbPath: string }}
 */
export function initProject(projectPath) {
  const squadDir = join(projectPath, '.squad');
  const dbPath = join(squadDir, 'tasks.db');

  if (!existsSync(squadDir)) {
    mkdirSync(squadDir, { recursive: true });
  }

  // Write .gitignore for .squad/
  const gitignorePath = join(squadDir, '.gitignore');
