...
<div class="flex flex-col h-full overflow-hidden" style="background: oklch(0.16 0.01 250);">
	...
	<div class="flex-none px-6 pt-5 pb-3" style="background: oklch(0.18 0.01 250); border-bottom: 1px solid oklch(0.25 0.02 250);">
		...
		<div class="flex items-center justify-between max-w-3xl mx-auto mt-3">
			<!-- Source tabs -->
			...
			{#if projects.length > 1}
				<select
					...
					class="text-xs px-2 py-1.5 rounded-md outline-none"
					style="
						background: oklch(0.14 0.01 250);
						border: 1px solid oklch(0.30 0.02 250);
	...
	<div class="flex-1 overflow-y-auto px-6 py-4">
		<div class="max-w-3xl mx-auto space-y-6">
			{#if loading}
				...
				<div class="space-y-4">
					{#each [1, 2, 3] as _}
						<div class="rounded-lg p-4" style="background: oklch(0.20 0.01 250); border: 1px solid oklch(0.25 0.02 250);">
							<div class="skeleton h-4 w-48 rounded mb-3" style="background: oklch(0.25 0.02 250);"></div>
							<div class="skeleton h-3 w-full rounded mb-2" style="background: oklch(0.22 0.02 250);"></div>
							<div class="skeleton h-3 w-2/3 rounded" style="background: oklch(0.22 0.02 250);"></div>
						</div>
					{/each}
				</div>
			{:else if error}
				<div class="rounded-lg p-4" style="background: oklch(0.22 0.08 25 / 0.15); border: 1px solid oklch(0.50 0.15 25 / 0.3);">
			...
			{:else}
					...
					<div class="flex items-center justify-between">
						...
						{#if meta.totalResults > 0}
							<button
								...
								class="text-xs px-3 py-1 rounded-md transition-all duration-150 flex items-center gap-1.5"
								style="
				...
				<!-- TASKS Section -->
				{#if (activeTab === 'all' || activeTab === 'tasks') && taskResults.length > 0}
					<div>
						<h3 class="text-xs font-semibold uppercase tracking-wider mb-2 flex items-center gap-2" style="color: oklch(0.55 0.02 250);">
							<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-3.5 h-3.5">
						...
						<div class="space-y-1.5">
							{#each taskResults as task}
								<button
									...
									class="w-full text-left rounded-lg px-4 py-3 transition-all duration-150 group"
									style="
								...
								>
									<div class="flex items-center gap-2 mb-1">
										...
										<span
											...
											style="background: {statusColor(task.status)}20; color: {statusColor(task.status)};"
										...
										{#if task.issue_type}
											<span class="text-[10px]" style="color: oklch(0.50 0.02 250);">{task.issue_type}</span>
										{/if}
									</div>
									<p class="text-sm font-medium truncate" style="color: oklch(0.88 0.02 250);">{task.title}</p>
				...
				<!-- MEMORY Section -->
				{#if (activeTab === 'all' || activeTab === 'memory') && memoryResults.length > 0}
					<div>
						<h3 class="text-xs font-semibold uppercase tracking-wider mb-2 flex items-center gap-2" style="color: oklch(0.55 0.02 250);">
							<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-3.5 h-3.5">
						...
						<div class="space-y-1.5">
							{#each memoryResults as mem}
								<button
									...
									class="w-full text-left rounded-lg px-4 py-3 transition-all duration-150"
									style="
										background: oklch(0.20 0.01 250);
										border: 1px solid oklch(0.25 0.02 250);
									"
								...
								>
									<div class="flex items-center gap-2 mb-1 flex-wrap">
										{#if mem.date}
											<span class="text-[10px] font-mono" style="color: oklch(0.50 0.02 250);">{mem.date}</span>
										{/if}
										{#if mem.taskId}
											<TaskIdBadge
												task={{ id: mem.taskId, status: 'closed' }}
												size="xs"
												minimal
											/>
										{/if}
										...
										{#if mem.section}
											<span
												...
												style="background: oklch(0.25 0.04 145 / 0.3); color: oklch(0.65 0.12 145);"
											>{mem.section}</span>
										{/if}
									</div>
									<p class="text-xs font-mono truncate" style="color: oklch(0.60 0.02 250);">{mem.file}</p>
									{#if mem.snippet}
										<p class="text-xs mt-1 line-clamp-2" style="color: oklch(0.55 0.02 250);">
				...
				<!-- FILES Section -->
				{#if (activeTab === 'all' || activeTab === 'files') && fileResults.length > 0}
					<div>
						<h3 class="text-xs font-semibold uppercase tracking-wider mb-2 flex items-center gap-2" style="color: oklch(0.55 0.02 250);">
							<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-3.5 h-3.5">
