 ...
 * 1. Fetching signal data from .jat/signals/{taskId}.jsonl
 * 2. Fetching task data from JAT Tasks
 * 3. Calling Claude Haiku to generate a structured summary
 *
 * Summary is cached in .jat/summaries/{taskId}.json after generation.
 */
...
export const GET: RequestHandler = async ({ params, url }) => {
	...
	try {
		...
		const summariesDir = join(projectRoot, '.jat', 'summaries');
		...
		try {
			const taskJson = execSync(`cd "${projectRoot}" && jt show "${id}" --json`, { encoding: 'utf-8' });
			...
			console.error('Failed to fetch task from JAT:', err);
		}

		if (!taskData) {
			return json({
				error: 'Task not found',
				message: `Could not find task ${id} in JAT`
			}, { status: 404 });
		}
