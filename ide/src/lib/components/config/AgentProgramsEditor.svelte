			...
			{#each programs as program (program.id)}
				...
				>
					...
					>
						...
						{#if AGENT_PRESETS.find((p) => p.id === program.id)?.authUrl && program.status?.commandAvailable && !program.status?.authConfigured}
							{@const presetAuthUrl = AGENT_PRESETS.find((p) => p.id === program.id)?.authUrl}
							<button
								class="btn btn-xs btn-ghost"
								onclick={(e) => { e.stopPropagation(); window.open(presetAuthUrl!, '_blank', 'noopener'); }}
