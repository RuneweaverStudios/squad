...
/**
 * Read JAT config to get custom project paths
 * @returns {Object|null} JAT config or null
 */
 ...
 * Scans both ~/code/ AND custom paths from ~/.config/jat/projects.json
...
export function getProjects() {
  ...
  // 1. First, check custom paths from JAT config (higher priority)
  /** @type {any} */
  ...
  // 2. Then scan ~/code/ for any projects not in JAT config
    ...
    try {
      ...
      for (const entry of entries) {
        if (entry.isDirectory()) {
          ...
          // Skip if already found via JAT config
