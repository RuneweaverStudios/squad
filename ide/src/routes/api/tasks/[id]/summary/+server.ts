 ...
 * 1. Fetching signal data from .squad/signals/{taskId}.jsonl
 * 2. Fetching task data from SQUAD Tasks
 * 3. Calling Claude Haiku to generate a structured summary
 *
 * Summary is cached in .squad/summaries/{taskId}.json after generation.
 */
...
export const GET: RequestHandler = async ({ params, url }) => {
	...
	try {
		...
		const summariesDir = join(projectRoot, '.squad', 'summaries');
		...
		try {
			const taskJson = execSync(`cd "${projectRoot}" && st show "${id}" --json`, { encoding: 'utf-8' });
			...
			console.error('Failed to fetch task from SQUAD:', err);
		}

		if (!taskData) {
			return json({
				error: 'Task not found',
				message: `Could not find task ${id} in SQUAD`
			}, { status: 404 });
		}
