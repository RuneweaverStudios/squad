# Ten ways to get tasks into JAT
...
JAT gives you ten different ingestion paths. Some are manual. Some are automated. One of them lets an AI agent plan the entire breakdown for you while you watch. Pick the one that matches where your work currently lives.
...
**CLI** is `jat create` from any terminal. Straightforward for scripting or when you're already in the terminal.

```bash
jat create "Fix OAuth timeout" \
  --type bug \
  --priority 1 \
  --labels security,auth \
  --description "Users hitting 60s timeout on login"
...
```
jat-auth (Epic: User Authentication)
  jat-auth.1: Set up Supabase auth config (P0)
  jat-auth.2: Create users table with RLS (P0)
  jat-auth.3: Google OAuth flow (P1, depends on .1, .2)
  jat-auth.4: Email/password flow (P1, depends on .2)
  jat-auth.5: Login UI components (P1, depends on .3, .4)
  jat-auth.6: Password reset (P2, depends on .4)
```
...
Whatever format your work lives in today, one of these ten paths will get it into JAT and in front of your agents.
