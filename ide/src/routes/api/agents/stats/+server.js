 ...
 *
 * Returns agent status breakdown from Agent Mail + SQUAD Tasks
 *
 * Response format:
 * {
 *   total: number,
...
export async function GET({ url }) {
	try {
		// Get all tasks from SQUAD
		const tasksJson = execSync('st list --json', {
			encoding: 'utf-8',
			cwd: process.env.HOME + '/code/squad'
		});
