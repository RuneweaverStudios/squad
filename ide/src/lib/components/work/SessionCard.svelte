<script lang="ts">
	...
	let minimapEnabled = $state(true); // Default ON, users can disable per-session
	...
	>
		...
		>
			...
			>
				...
				<!-- Text input: [attach][settings▼][keybd▼][copy] LEFT | input MIDDLE | [action buttons] RIGHT -->
				<div class="flex gap-1.5 items-end">
					...
					<div class="flex items-center gap-0.5 flex-shrink-0 pb-0.5">
						...
						<!-- Settings dropdown (hover to show autoscroll, livestream, minimap toggles) -->
						<div class="dropdown dropdown-hover dropdown-top">
							<button
								tabindex="0"
								class="btn btn-xs btn-ghost"
								title="Terminal settings (hover for options)"
							>
								<svg
									class="w-3 h-3"
									fill="none"
							...
							<ul
								tabindex="0"
								class="dropdown-content z-[100] menu menu-xs p-1 shadow-lg bg-base-200 rounded-box w-auto min-w-[140px]"
							>
								<li>
									<button
									...
									>
										<svg
											class="w-3 h-3 transition-colors {autoScroll ? 'text-primary' : 'opacity-40'}"
											fill="none"
											viewBox="0 0 24 24"
											stroke="currentColor"
											stroke-width="2"
										...
										{#if autoScroll}
											<span class="text-primary text-[9px] ml-auto">ON</span>
										{/if}
									</button>
								</li>
								<li>
									...
									>
										<svg
											class="w-3 h-3 transition-colors {liveStreamEnabled ? 'text-info' : 'opacity-40'}"
											fill="none"
											viewBox="0 0 24 24"
											stroke="currentColor"
											stroke-width="2"
										...
										{#if liveStreamEnabled}
											<span class="text-info text-[9px] ml-auto">ON</span>
										{/if}
									</button>
								</li>
								{#if mode === 'agent'}
									<li>
										...
										>
											<svg
												class="w-3 h-3 transition-colors {minimapEnabled ? 'text-info' : 'opacity-40'}"
												fill="none"
												viewBox="0 0 24 24"
												stroke="currentColor"
												stroke-width="2"
						...
						<button
							class="btn btn-xs btn-ghost"
							onclick={copySessionContents}
							title={sessionCopied ? "Copied!" : "Copy session output to clipboard"}
							disabled={!output}
						>
							{#if sessionCopied}
								<svg
