#!/bin/bash
#
# Migration: Rename jomarchy-agent-tools-* tasks to squad-*
#
# This script updates task IDs in the squad tasks database:
# - Changes all jomarchy-agent-tools-XXX -> squad-XXX
...
DB_PATH="$HOME/code/squad/.squad/tasks.db"
BACKUP_PATH="$HOME/code/squad/.squad/tasks.db.backup-$(date +%Y%m%d-%H%M%S)"

echo "🔍 SQUAD Tasks Database Migration: jomarchy-agent-tools → squad"
echo ""
...
-- Update issues table: jomarchy-agent-tools-XXX → squad-XXX
UPDATE issues
SET id = 'squad-' || substr(id, length('jomarchy-agent-tools-') + 1)
WHERE id LIKE 'jomarchy-agent-tools-%';

-- Update dependencies table: issue_id column
UPDATE dependencies
SET issue_id = 'squad-' || substr(issue_id, length('jomarchy-agent-tools-') + 1)
WHERE issue_id LIKE 'jomarchy-agent-tools-%';

-- Update dependencies table: depends_on_id column
UPDATE dependencies
SET depends_on_id = 'squad-' || substr(depends_on_id, length('jomarchy-agent-tools-') + 1)
...
SELECT COUNT(*) || ' tasks now have squad- prefix' as result
FROM issues
WHERE id LIKE 'squad-%'
...
echo ""
echo "📊 Summary:"
sqlite3 "$DB_PATH" "SELECT COUNT(*) FROM issues WHERE id LIKE 'squad-%';" | while read count; do
    echo "  Total squad-* tasks: $count"
done
...
echo ""
echo "🎉 Done! Refresh your IDE to see the changes."
