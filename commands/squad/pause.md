...
**Usage:**
- `/squad:pause` - Pause the current session (kills tmux, preserves session ID for resume)
...
**When to use:**
- After sending a reply via `squad-signal reply` — pause so the session can resume when the user responds
...
TASK_ID=$(cat /tmp/squad-signal-tmux-${TMUX_SESSION}.json 2>/dev/null | jq -r '.taskId // empty' 2>/dev/null)
if [ -z "$TASK_ID" ]; then
