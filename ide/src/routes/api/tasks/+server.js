...
import { getTasks, getProjects, getTaskById, createTask, updateTask, getScheduledTasks } from '$lib/server/squad-tasks.js';
	...
	const responseData = await singleFlight(key, async () => {
		...
		if (search && search.trim()) {
			...
			tasks = tasks.filter(task => {
				...
				// e.g. searching "squad-abc" should not match "squad-abc.1", "squad-abc.2")
...
export async function POST({ request }) {
	...
	try {
		...
		if (project) {
			...
			if (!projectInfo.exists) {
				return json(
					{
						error: true,
						message: `Project directory not found: ${projectInfo.path}. Either create the directory and run 'st init', or add the project to ~/.config/squad/projects.json with the correct path.`,
		...
		if (message.includes('No .squad/ database') || message.includes('Run initProject()')) {
			const projectName = project || 'this project';
			return json(
				{
					error: true,
					message: `Project "${projectName}" has not been initialized for task tracking. Run "st init" in the project directory, or use the "Add Project" button on the Projects page.`,
					type: 'project_not_initialized',
					hint: `cd ~/code/${projectName} && st init`
				},
				{ status: 400 }
			);
		}
