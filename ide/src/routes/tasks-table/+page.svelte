<script lang="ts">
	 ...
	 * Tasks Table Page
	 *
	 * Combines the full /tasks session view with TaskTable below.
	 * Top: Session management (from /tasks page)
	 * Bottom: TaskTable for browsing/managing tasks
	 */
	...
	let taskTableSelectedTaskId = $state<string | null>(null);
	let completedTasksFromActiveSessions = $state<Set<string>>(new Set());
	...
	// Fetch TaskTable data
	async function fetchTaskTableData() {
		try {
			// Fetch all tasks - TaskTable handles filtering internally
			const response = await fetch('/api/tasks');
			if (!response.ok) throw new Error('Failed to fetch tasks');
			const data = await response.json();
			taskTableTasks = data.tasks || [];
			taskTableAllTasks = data.tasks || [];

			taskTableError = null;
		} catch (err) {
			taskTableError = err instanceof Error ? err.message : 'Unknown error';
		} finally {
	...
	function handleTaskTableTaskClick(taskId: string) {
		taskTableSelectedTaskId = taskId;
		taskTableDrawerOpen = true;
	}

	function handleTaskTableAgentClick(agentName: string) {
		// Find and expand that agent's session
		const session = sessions.find(s => s.name === `squad-${agentName}`);
		if (session) {
			toggleExpanded(session.name);
		}
	}

	function handleTaskTableTasksChanged() {
		fetchTaskTableData();
	}

	function handleSplitResize(deltaY: number) {
		const containerHeight = window.innerHeight - 48;
		const deltaPercent = (deltaY / containerHeight) * 100;
		const newPercent = splitPercent + deltaPercent;
		splitPercent = Math.max(MIN_SPLIT, Math.min(MAX_SPLIT, newPercent));
	}
	...
	onMount(() => {
		...
		// Poll every 3 seconds for sessions
...
<svelte:head>
	<title>Tasks Table | SQUAD IDE</title>
	<meta name="description" content="Sessions view with task table below - review tasks, spawn sessions, and track agent progress." />
	<meta property="og:title" content="Tasks Table | SQUAD IDE" />
	<meta property="og:description" content="Sessions view with task table below - review tasks, spawn sessions, and track agent progress." />
...
<div class="tmux-page" style="background: oklch(0.14 0.01 250);">
	...
	<!-- Split layout: Sessions (top) + TaskTable (bottom) -->
	<div class="split-container" style="height: calc(100vh - 100px);">
		<!-- Top panel: Sessions -->
		<div class="split-panel sessions-panel" style="height: {sessionsCollapsed ? '40px' : splitPercent + '%'}; overflow: {sessionsCollapsed ? 'hidden' : 'auto'};">
			{#if sessionsCollapsed}
				<button class="collapsed-header" onclick={toggleSessionsCollapse}>
											...
											<StatusActionBadge
												...
												onAction={async (actionId) => {
													if (actionId === 'attach') {
														...
														// Send /squad:complete command to session
														...
														await fetch(`/api/work/${sessionId}/input`, {
															method: 'POST',
															headers: { 'Content-Type': 'application/json' },
															body: JSON.stringify({ input: '/squad:complete', type: 'text' })
														});
														...
														// Send /squad:complete --kill command to session
														// Claude Code slash commands need: Ctrl+U clear, text, extra Enter
														const sessionId = encodeURIComponent(session.name);
														await fetch(`/api/work/${sessionId}/input`, {
															method: 'POST',
														...
														await fetch(`/api/work/${sessionId}/input`, {
															method: 'POST',
															headers: { 'Content-Type': 'application/json' },
															body: JSON.stringify({ input: '/squad:complete --kill', type: 'text' })
														});
														await new Promise(r => setTimeout(r, 100));
														await fetch(`/api/work/${sessionId}/input`, {
															method: 'POST',
	...
	{/if}
		...
		<ResizableDivider
			...
			onCollapsedClick={toggleSessionsCollapse}
		/>

		<!-- Bottom panel: TaskTable -->
		<div class="split-panel tasktable-panel" style="height: {sessionsCollapsed ? 'calc(100% - 48px)' : (100 - splitPercent) + '%'};">
			{#if taskTableLoading}
				<TaskTableSkeleton rows={8} />
			{:else if taskTableError}
				<div class="tasktable-error">
					<p>{taskTableError}</p>
					<button class="btn btn-sm btn-primary" onclick={fetchAllTaskTableData}>Retry</button>
				</div>
			{:else}
				<TaskTable
	...
	<!-- TaskDetailDrawer for TaskTable -->
	<TaskDetailDrawer
		bind:isOpen={taskTableDrawerOpen}
		bind:taskId={taskTableSelectedTaskId}
	/>
</div>
