<script lang="ts">
	/**
	 * Dashboard Page - Flat Hierarchy Layout
	 *
	...
	const logger = createNamespacedLogger('ui:dash', { component: 'DashPage' });
	...
	async function handleAttachTerminal(sessionName: string) {
		try {
			const response = await fetch(`/api/dash/${sessionName}/attach`, { method: 'POST' });
			if (!response.ok) {
