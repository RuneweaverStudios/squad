...
SQUAD Tasks is the task system. Its a lightweight, dependency-aware task database stored in SQLite. Each project gets its own `.squad/` directory, and the `st` CLI handles everything from creation to completion.

## Creating tasks

```bash
st create "Add dark mode toggle to settings page" \
  --type feature \
  --priority 1 \
...
|--------|---------|--------|
| `open` | Available to start | `st create` |
| `in_progress` | An agent is working on it | `/squad:start` |
| `blocked` | Waiting on a dependency or external factor | `st update --status blocked` |
| `closed` | Work complete | `/squad:complete` |

**Use underscores, not hyphens.** `in_progress` works. `in-progress` does not.

```bash
# Update status manually
st update myproject-abc --status in_progress --assignee CalmMeadow

# Close a task
st close myproject-abc --reason "Implemented and tested"
```
...
| `chat` | Conversational threads from external channels (typically handled with `/squad:chat`) |

## Dependencies

SQUAD tracks dependencies between tasks. A task with unmet dependencies shows as `blocked` and wont appear in `st ready` output.

```bash
# Task B depends on Task A (B cant start until A is closed)
st dep add myproject-bbb myproject-aaa

# View dependency tree
st dep tree myproject-bbb

# Remove a dependency
st dep remove myproject-bbb myproject-aaa
...
```bash
st create "Build OAuth login page" --type task --deps myproject-aaa
```

## Finding ready work

The `st ready` command returns tasks that are `open` and have all dependencies satisfied:

```bash
st ready --json
```

This is what agents use during `/squad:start auto` to pick their next task. The output is sorted by priority, so P0 tasks come first.
...
# Filter by labels
st list --status open --labels frontend
```
...
```bash
# 1. Create the epic
st create "User authentication system" \
  --type epic \
  --priority 1 \
...
st dep add myproject-abc myproject-def
st dep add myproject-abc myproject-ghi
st dep add myproject-abc myproject-jkl
```

Do NOT use the `--parent` flag. It creates dependencies in the wrong direction (children blocked by parent). Use `st dep add` or the helper script `st-epic-child` instead.

```bash
# Safe helper that gets the direction right
st-epic-child myproject-abc myproject-def
```
...
Each project has its own `.squad/` directory and task namespace. Task IDs include the project prefix so theres never ambiguity.
...
# Work in the squad project
cd ~/code/squad
st ready                    # Shows only squad tasks
```
...
The `.squad/` directory contains both committed files and local-only files:
...
Do not add `.squad/` to your root `.gitignore`. The `.squad/.gitignore` file handles excluding the SQLite database.

## Next steps

- [Agent Registry](/docs/agent-registry/) - Agent identity for multi-agent coordination
- [Workflow Commands](/docs/workflow-commands/) - How /squad:start and /squad:complete use SQUAD Tasks
