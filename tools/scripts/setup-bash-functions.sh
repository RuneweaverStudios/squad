...
# - Generate squad-<project> launcher functions for each repo in ~/code/
...
# Auto-generated by SQUAD installer
# Launch Claude Code with /squad:start for each project
...
for repo_dir in "$CODE_DIR"/*; do
    ...
    # Create function name: squad- + repo name (lowercase)
    # e.g., squad -> squad-squad, my-project -> squad-myproject
    FUNC_NAME="squad-$(echo "$REPO_NAME" | tr '[:upper:]' '[:lower:]' | tr -d '-')"

    echo -e "  ${GREEN}✓${NC} $FUNC_NAME() -> ~/code/$REPO_NAME"

    FUNCTIONS_BLOCK+="$FUNC_NAME() {
    local session_name=\"squad-pending-\$\$\"
    local logs_dir=~/code/$REPO_NAME/.squad/logs
    local log_file=\"\$logs_dir/session-\${session_name}-\$(date +%Y%m%d-%H%M%S).log\"

    cd ~/code/$REPO_NAME
    mkdir -p \"\$logs_dir\"

    # Create tmux session, run Claude - session gets renamed by /squad:start
    ...
    sleep 0.3
    tmux send-keys -t \"\$session_name\" \"AGENT_MAIL_URL=http://localhost:8765 claude --dangerously-skip-permissions '/squad:start' \$*\" Enter
...
# Remove existing SQUAD block if present
if grep -q "$MARKER_START" "$SHELL_CONFIG"; then
    echo "  → Removing existing SQUAD launcher block..."
